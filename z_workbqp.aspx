			...
			function q_gfPost() {
				$('#q_report').q_report({
					...
					options : [{
						...
						type : '5', //[10]
						name : 'xwhere',//f,u,q
						value : '3@階段一：廠內製令單(應開工日),1@階段二：已送驗未入庫(送驗入庫日),2@階段三：已驗收入庫(送驗入庫日)'.split(',')
					},{
						type : '1', //[11] [12]
						name : 'xcuadate'
					}, {
				...
				if(!(q_getPara('sys.project').toUpperCase()=='JO'|| q_getPara('sys.project').toUpperCase()=='AD')){
					for(var i=0;i<$('#q_report').data().info.reportData.length;i++){
						if(
                        	...
                        	$('#q_report').data().info.reportData[i].report=='z_workbqp_jo04' || //報廢單
                        	$('#q_report').data().info.reportData[i].report=='z_workbqp_jo05' //可用尾箱明細表
						){
