...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr">
	<head>
		...
		<script type="text/javascript">
			...
			var q_name = "rack";
			var q_readonly = ['txtStore','txtWorker'];
			var bbmNum = [];
			var bbmMask = [];
			q_sqlCount = 6;
			brwCount = 6;
				...
				$('#txtNoa').change(function(e) {
					...
					if ($(this).val().length > 0) {
						if ((/^(\w+|\w+\u002D\w+)$/g).test($(this).val())) {
							...
							var t_where = "where=^^ ltrim(rtrim(noa))='" + t_noa + "' ^^";
							q_gt('rack', t_where, 0, 0, 0, "checkNoa_change", r_accy);
						} else {
							Lock();
			...
			function q_gtPost(t_name) {
				switch (t_name) {
					case 'checkNoa_change':
						var as = _q_appendData("rack", "", true);
						if (as[0] != undefined) {
							alert('已存在 ' + as[0].noa + ' ' + as[0].storeno + ' ' + as[0].store + ' ' + as[0].place);
							Unlock();
						}
						break;
					case 'checkNoa_btnOk':
			...
			function btnOk() {
				...
				var t_err = q_chkEmpField([['txtNoa', q_getMsg('lblNoa')], ['txtStoreno', q_getMsg('lblStoreno')]]);
				if (t_err.length > 0) {
					alert(t_err);
					Unlock();
					return;
		...
		<div id='dmain' style="overflow:hidden;">
			<div class="dview" id="dview" style="float: left;"  >
				<table class="tview" id="tview"   border="1" cellpadding='2'  cellspacing='0' style="background-color: #FFFF66;">
					<tr>
						...
						<td align="center" style="width:30%"><a id='vewRack'></a></td>
						<td align="center" style="width:35%"><a id='vewStoreno'></a></td>
					</tr>
					<tr>
						<td><input id="chkBrow.*" type="checkbox" style=''/></td>
						<td align="center" id='noa'>~noa</td>
						<td align="center" id='rack'>~rack</td>
						<td align="center" id='storeno store'>~storeno ~store</td>
					</tr>
				</table>
			</div>
			<div class='dbbm' style="float: left;">
				<table class="tbbm"  id="tbbm"   border="0" cellpadding='2'  cellspacing='5'>
					...
					<tr>
						<td class="td1"><span> </span><a id='lblRack' class="lbl"></a></td>
						<td class="td2"><input id="txtRack"  type="text" class="txt c1" /></td>
						<td class="td3"></td>
						<td class="td4"></td>
					</tr>
					<tr>
