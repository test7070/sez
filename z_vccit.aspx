			...
			function q_gfPost() {///要改裡面條件，請告知經理。(發送email用)
				$('#q_report').q_report({
					...
					options : [{
						...
						type : '5',
						name : 'xtype', //[11]
						value : [q_getPara('report.all')].concat((q_getPara('ucc.typea')+','+q_getPara('uca.typea')).split(','))//[q_getPara('report.all')].concat((q_getPara('sys.comp').indexOf('英特瑞')>-1 || q_getPara('sys.comp').indexOf('安美得')>-1)?q_getPara('ucc.typea_it').split(','):(q_getPara('ucc.typea')+','+q_getPara('uca.typea')).split(','))
	                }, {
						type : '5',
						name : 'xstype', //[12]
