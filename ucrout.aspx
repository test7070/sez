	...
	<head>
		...
		<script type="text/javascript">
                        ...
                        if (as[0] != undefined) {
                        	...
                        	if (ass[0] != undefined) {
                        		for (var i = 0; i < ass.length; i++) {
                        			ass[i].mount=round(q_add(dec(ass[i].mount),q_div(q_mul(dec(ass[i].mount),dec(ass[i].loss)),100)),q_getPara('vcc.mountPrecision'));
                        			ass[i].weight=round(q_add(dec(ass[i].weight),q_div(q_mul(dec(ass[i].weight),dec(ass[i].loss)),100)),q_getPara('vcc.weightPrecision'));
                        		}
			...
			function _btnSeek() {
				if (q_cur > 0 && q_cur < 4)
					return;
				q_box('ucrout_s.aspx', q_name + '_s', "500px", "320px", $('#btnSeek').val());
			}
