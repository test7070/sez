    ...
    <script type="text/javascript">
       ...
       function main() {
			...
			$('#txtNoa').change(function(e){
					...
					if($(this).val().length>0){
						if((/^(\w+|\w+\u002D\w+)$/g).test($(this).val())){
                    		...
                    		q_gt('boat', t_where, 0, 0, 0, "checkBoatno_change", r_accy);
						}else{
							Lock();
        ...
        function q_gtPost(t_name) {  
            switch (t_name) {
               case 'checkBoatno_change':
                		var as = _q_appendData("boat", "", true);
                        if (as[0] != undefined){
                        	alert('已存在 '+as[0].noa+' '+as[0].boat);
                        }
                		break;
                case 'checkBoatno_btnOk':
                		var as = _q_appendData("boat", "", true);
                        if (as[0] != undefined){
                        	alert('已存在 '+as[0].noa+' '+as[0].boat);
                            Unlock();
        ...
        function _btnSeek() {
            if (q_cur > 0 && q_cur < 4)  // 1-3
                return;
            q_box('boat_s.aspx', q_name + '_s', "500px", "310px", q_getMsg( "popSeek"));
        }
        function btnIns() {
            _btnIns();
            refreshBbm();
        ...
        function btnOk() {
        	 Lock(); 
				...
				if(q_cur==1){
                    ...
                    q_gt('boat', t_where, 0, 0, 0, "checkBoatno_btnOk", r_accy);
                }else{
                	wrServer($('#txtNoa').val());
                }
                   }
...
<!--#include file="../inc/toolbar.inc"-->
        ...
        <table class="tbbm"  id="tbbm"   border="0" cellpadding='2'  cellspacing='5'>
          <tr class="tr1">
            ...
            <tr class="tr2">
               <td class="td1"><span> </span><a id="lblBoat" class="lbl"></a></td>
