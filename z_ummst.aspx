		...
		aPop = new Array(['txtXpart', '', 'part', 'noa,part', 'txtXpart', "part_b.aspx"]);
			...
			function q_gfPost() {
			  $('#q_report').q_report({
				...
				var TmpObj = $('#q_report').data('info').reportData;
				for(var i=0;i<TmpObj.length;i++){
					if(TmpObj[i].report.substring(0,6) == 'z_vccj'){
						if(q_getPara('sys.project').toUpperCase() == 'ST'){
							TmpObj[i].report = 'z_vccj';
						}else if(q_getPara('sys.project').toUpperCase() == 'PI'){
							TmpObj[i].report = 'z_vccjpi';
						}
					}
				}
				q_popAssign();
