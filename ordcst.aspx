...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr">
	<head>
		...
		<script type="text/javascript">
            ...
            , ['txtTggno', 'lblTgg', 'tgg', 'noa,comp,ncik,paytype,tel,fax,addr_fact,zip_fact', 'txtTggno,txtTgg,txtNick,txtPaytype,txtTel,txtFax,txtAddr,txtPost', 'tgg_b.aspx']);
            brwCount2 = 10;
                ...
                $('#lblOrdb').click(function() {
                    ...
                    if (t_tggno.length > 0) {
                        if (t_ordbno.length > 0)
                            t_where = "enda=0 && " + (t_tggno.length > 0 ? q_sqlPara("tggno", t_tggno) : "") + "&& " + (t_ordbno.length > 0 ? q_sqlPara("noa", t_ordbno) : "") + " && kind='" + $('#cmbKind').val() + "'";
                        ////  sql AND 語法，請用 &&
                        else
                            t_where = "enda=0 && " + (t_tggno.length > 0 ? q_sqlPara("tggno", t_tggno) : "") + " && kind='" + $('#cmbKind').val() + "'";
                        ////  sql AND 語法，請用 &&
                        t_where = t_where;
                    } else {
                ...
                switch (b_pop) {
                    case 'ordbs':
                        if (q_cur > 0 && q_cur < 4) {
                            ...
                            ret = q_gridAddRow(bbsHtm, 'tbbs', 'txtProductno,txtProduct,txtSpec,txtDime,txtWidth,txtLengthb,txtRadius,txtOrdbno,txtNo3,txtPrice,txtMount,txtWeight,txtTotal,txtMemo,txtTheory,txtStyle,txtClass,txtUno,txtSize', b_ret.length, b_ret, 'productno,product,spec,dime,width,lengthb,radius,noa,no3,price,mount,weight,total,memo,theory,style,class,uno,size', 'txtProductno,txtProduct,txtSpec');
