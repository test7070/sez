			...
			for (var i = 0; i < q_bbtCount; i++) {
				...
				if (!$('#btnMinut__' + i).hasClass('isAssign')) {
					$('#txtPrice__'+i).change(function() {
						...
						q_bodyId($(this).attr('id'));
						b_seq = t_IdSeq;
						if(emp($('#txtWages__'+b_seq).val())&&emp($('#txtMakes__'+b_seq).val())&&!emp($('#txtPrice__'+b_seq).val())){
							q_tr('txtWages_fee__'+b_seq,q_float('txtPrice__'+b_seq)*0.8);
							q_tr('txtMakes_fee__'+b_seq,q_float('txtPrice__'+b_seq)*0.2);
						}
						if(!emp($('#txtWages__'+b_seq).val())&&!emp($('#txtPrice__'+b_seq).val())){
							q_tr('txtWages_fee__'+b_seq,q_float('txtPrice__'+b_seq)*(q_float('txtWages__'+b_seq)/100));
						}
						if(!emp($('#txtMakes__'+b_seq).val())&&!emp($('#txtPrice__'+b_seq).val())){
							q_tr('txtMakes_fee__'+b_seq,q_float('txtPrice__'+b_seq)*(q_float('txtMakes__'+b_seq)/100));
						}
					});
