        	...
        	$('#txtNoa').change(function(e){
					...
					if($(this).val().length>0){
						if((/^(\w+|\w+\u002D\w+)$/g).test($(this).val())){
                    		...
                    		q_gt('factory', t_where, 0, 0, 0, "checkFactoryno_change", r_accy);
						}else{
							Lock();
							alert('編號只允許 英文(A-Z)、數字(0-9)及dash(-)。'+String.fromCharCode(13)+'EX: A01,BA01-001');
							Unlock();
						}
					}
                });
        ...
        function q_gtPost(t_name) {  
            switch (t_name) {
               case 'checkFactoryno_change':
                        ...
                        if (as[0] != undefined){
                        	alert('已存在'+as[0].noa+' '+as[0].factory);
                        }
                		break;
